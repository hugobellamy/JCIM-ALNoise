...
def test(path):
    ...
    for i in noise_factors:
        ...
        for j in range(repeats):
            ...
            fname = 'results_10%/'+CHEMBL_name+'/noR/AL_noise'+str(i)+'R'+str(j)+'.pkl'
            ...
            fname = 'results_10%/'+CHEMBL_name+'/withR/AL_noise'+str(i)+'R'+str(j)+'.pkl'
    
            pickle.dump(all_hits, open(fname,'wb'))

        
def exp(name):
    length = len(pd.read_csv('data/'+name))
    if length>800:  
        CHEMBL_name = name[:-4]
        print('New Dataset '+CHEMBL_name)
        os.mkdir('results_10%/'+CHEMBL_name)
        os.mkdir('results_10%/'+CHEMBL_name+'/noR')
        os.mkdir('results_10%/'+CHEMBL_name+'/withR')
        test(name)
