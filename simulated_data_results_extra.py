...
name = 'friedman'
source = f'results_simulated_C/{name}/noR/'
sourceR = f'results_simulated_C/{name}/wR/'
...
def figure2():
    ...
    print(res)
    for i in labels:
        y = np.mean(res[i], axis=1)
        print(y)
        plt.plot(x, y, 'o',label=i, color=colors[i],)
    plt.legend(loc=8, ncol=3)
    plt.ylim(-20,240)
    plt.xlabel(r'noise fraction ($\alpha$)')
    plt.ylabel('hits')
    plt.savefig(f'figures/simulated_{name}/fig2.png', dpi=600)
    plt.show()
...
def figure4():
    ...
    for i in noise_levels:
        ...
        x = np.linspace(1, length, length)
        for j in labels:
            plt.plot(x, np.mean(data[j], axis=0),
                          label=j, color=c2[j])
            plt.plot(x, np.mean(data2[j], axis=0),
                          label=j+'(retests)', color=c3[j])
            
        plt.legend(loc=8, ncol=2)
        plt.xlabel('batch number')
        # plt.xlim(-0,12)
        plt.ylabel('true hits')
        plt.savefig(f'figures/simulated_{name}/fig4-'+str(i)+'.png', dpi=600)
        plt.show()
